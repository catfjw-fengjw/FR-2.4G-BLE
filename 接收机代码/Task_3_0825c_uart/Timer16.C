...
void rf_adv_data_config(void)
{
	...
	TXbufData[8] = 0x02;    // 广播标志段长度 2
	TXbufData[9] = 0x01;    // 类型：标志
	TXbufData[10] = 0x06;   // 标志内容

    TXbufData[11] = 0x09;  // 名称段长度 9
    TXbufData[12] = 0x09;                     // 类型：设备名称
	TXbufData[13] = 0x4C; // L
	TXbufData[14] = 0x58; // X
	TXbufData[15] = 0x31; // 方案商代号
	TXbufData[16] = 0x31; // 设备代号
	TXbufData[17] = 0x31; // 客户代码
	TXbufData[18] = 0x31; // 0
	TXbufData[19] = 0x31; // 0
	TXbufData[20] = 0x31; // 1

	TXbufData[21] = 0x11; //17
	TXbufData[22] = 0xff; // 
	TXbufData[23] = 0x0; // 
	TXbufData[24] = 0x0; // 
	TXbufData[25] = 0x0; // 
	...
	TXbufData[10] 	= 0x4C; // L
	TXbufData[11] 	= 0x59; // Y
	TXbufData[12] 	= 0x31; // 方案商代号
	TXbufData[13] 	= 0x31; // 设备代号
	TXbufData[14] 	= 0x31; // 客户代码
	TXbufData[15] 	= 0x31; // 0
	TXbufData[16] 	= 0x31; // 0
	TXbufData[17] 	= 0x31; // 1

	TXbufData[18] = 0x14; //20
	TXbufData[19] = 0xff; // 
	TXbufData[20] = 0x0; // 
	TXbufData[21] = 0x0; // 
	TXbufData[22] = TempWorkMode; // 模式
	TXbufData[23] = 0x0; // 震动强度
	TXbufData[24] = 0x0; //	拍打强度
	TXbufData[25] = 0x0; //	吮吸强度
	TXbufData[26] = 0x0; // 夹吸档位
	TXbufData[27] = 0x0; // 电击强度
	TXbufData[28] = 0x0; // 加热强度
	TXbufData[29] = 0x0; // 
	TXbufData[30] = 0x0; // 
...
// 42 25 2F DC E0 20 8C 5C 09 09 4C 58 31 31 31 31 31 31 14 FF 00 00 31 2A 1E 38 03 00 24 00 00 00 00 00 00 00 00 00 00 D3 37 A8 
				...
				while (Rx_i < RX_len)
				{
					...
					if(blecrc == calc_crc)
					{
						...
						if(TempWorkMode != RXbufData[22])
						{
							...
							TempWorkMode = RXbufData[22];
							JudgeMode_Receive(TempWorkMode);
						}
#if 0
					UART_Send_Byte( RXbufData[22]);
#endif						
					}
				}
				while(RX_k < BUFF_LEN)  // 
